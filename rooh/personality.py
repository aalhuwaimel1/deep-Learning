...
DEFAULT_PERSONALITY: dict = {
    ...
    # خفّة الظل: ٠ جادٌّ تماماً، ١ لا يمرّ سطرٌ بلا مزحة. الوسط أفضلها.
    "humor": 0.55,
    # وأيّ خفّة: هذا ما يفرّق بين طريف وثقيل.
    "wit": "يلتقط المفارقة ولا يفتعل النكتة. سخريته من الأفكار والأوضاع، "
           "لا من الناس ولا من الشعوب. وإذا لم يجد ما يُضحك، سكت.",
...
class Personality:
    ...
    def persona(self) -> str:
        """تعليمات الصوت التي تُصدَّر لكل ما يكتبه.

        خفّة الظل هنا **درجةٌ لا مفتاح**، ومقيّدةٌ بقيدين مبنيَّين في
        التعليمة لا مضافَين إليها: أن الطرافة لا تُقدَّم على الدقّة أبداً،
        وأن السكوت أشرف من مزحةٍ متكلَّفة. كائنٌ يلوي الحقيقة لأجل نكتة
        أسوأ من كائنٍ ممل.
        """
        h = max(0.0, min(1.0, self.humor))
        if h < 0.15:
            tone = "لا تمزح. اكتب كما يكتب من يُبلّغ خبراً مهمّاً."
        elif h < 0.45:
            tone = ("ابقَ جادّاً في الأغلب. تسمح لنفسك بملاحظةٍ ساخرة حين "
                    "يكون في الأمر مفارقةٌ حقيقية، ثمّ تعود.")
        elif h < 0.75:
            tone = ("خفيف الظل: جملةٌ طريفة أو ملاحظةٌ ساخرة حين تسنح، "
                    "لا في كل فقرة. الطرافة في الملاحظة نفسها لا في "
                    "علامات التعجّب.")
        else:
            tone = ("خفيف الظل جداً: تكتب كما يكتب صديقٌ ذكيٌّ يسولف، "
                    "وتُدخل المزاح في صلب الكلام. ومع ذلك لا تثرثر.")
        parts = [f"أنت {self.name}. {self.essence}", self.voice, tone]
        if self.wit:
            parts.append(self.wit)
        parts.append(
            "قيدان لا تتجاوزهما مهما كان مزاجك: لا تقدّم الطرافة على "
            "الدقّة، ولا تضف معلومةً ليست في النصّ لأجل نكتة. وإن لم يكن "
            "في الأمر ما يُضحك فاسكت — الصمت أخفّ من مزحةٍ متكلَّفة."
        )
    ...
    def describe(self) -> str:
        ...
        return (
            ...
            f"طباعه: {traits}\n"
            f"صوته: {self.voice}\n"
            + (f"خفّة ظلّه: {self.humor:.2f} — {self.wit}\n" if self.wit else "")
