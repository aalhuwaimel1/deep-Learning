...
The system it describes exists as version 0.1 (about 4,000 lines of Python, standard library only, 131 offline tests). It has **not yet wandered the real multilingual web**. One live external fetch has now been exercised end-to-end — robots.txt check, HTTPS GET, text extraction, digestion, storage, drive update, recall — against `raw.githubusercontent.com` (219 ms, 8.9 KB, keywords and summary produced, novelty 1.00 → learning 0.00 on an empty body, as designed). That validates the plumbing on one real host; it does not validate wandering, language selection, feeds, research databases, or anything measured over time. Every other number in this document comes from code or from runs against a local mock network. The multilingual run remains the first real experiment.
...
**5.8 Manners.** Honest User-Agent, `robots.txt` respected — and its *failure* modes distinguished, which is where the guarantee actually lives. An earlier version assumed permission whenever `robots.txt` could not be fetched, so a host answering 403 to `robots.txt` would then be crawled as though it had consented: the safeguard collapsed precisely when it was needed. Per RFC 9309 the code now treats a missing file (404) as unrestricted, a refusal to serve it (401/403) as a full disallow, and a server error or network failure as a temporary disallow. Found by attempting a real fetch, not by reading the code. Six regression tests cover the modes. no host hit faster than once per two seconds, no paywalls bypassed, no logins, no personal data collected. Owner-configurable `hosts_blocked` and `terms_blocked`.

**5.9 Modules and size.** body 519 lines, wanderer 564, drives 180, insight 245, mind 211, lang 257, languages 154, research 256, sources 267, personality 172, senses 156, net 150, cli 824 (21 commands); tests 1,322 lines, 107 tests, all offline. Standard library only; `anthropic` (summarizing into Arabic regardless of source language, journal in character voice, cross-world comparison prose) and `beautifulsoup4` are optional.
...
- 44% of extracted Chinese keywords are cross-boundary fragments (§5.3). Every measurement that depends on CJK keyword quality — novelty, boredom, gaps in CJK languages — carries that noise.
